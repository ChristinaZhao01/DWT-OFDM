...
for EsN0=ber_begin:ber_end
    ...
    for n=1:N_OFDM_Frame
        ...
        %%  信道编码（卷积码编码）
        trellis = poly2trellis(9,[765 671 513 473]);       %(4,3,3)卷积编码
        ...
        Tx_TimeDomain = Tx_TimeDomain.';                %并/串变换
        %% 通过瑞利信道 
        ...
        Rx_TimeDomain = Rx_TimeDomain.';        %串并变换
    ...
    SER_Th_QPSK(EsN0+1)=1-(1-(3/4)*erfc(sqrt((3/2)*10^(EsN0/10)/(16-1))))^2;%对每一个信噪比计算理论误码率
    N=length(snr_draw);
